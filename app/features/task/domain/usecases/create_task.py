...
class CreateTaskUseCase(BaseUseCase[Tuple[TaskCreateModel], TaskReadModel]):
    ...
    @abstractmethod
    async def __call__(self, args: Tuple[TaskCreateModel]) -> TaskReadModel:
...
class CreateTaskUseCaseImpl(CreateTaskUseCase):
    ...
    async def __call__(self, args: Tuple[TaskCreateModel]) -> TaskReadModel:
        ...
        try:
            await self.unit_of_work.repository.create(task)
        except Exception as _e:
            await self.unit_of_work.rollback()
            raise

        await self.unit_of_work.commit()
        created_task = (await self.unit_of_work.repository.find_by_owner_id(task.owner_id))[0]
