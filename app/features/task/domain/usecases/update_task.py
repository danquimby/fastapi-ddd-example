...
class UpdateTaskUseCase(BaseUseCase[Tuple[int, TaskUpdateModel], TaskReadModel]):
    ...
    @abstractmethod
    async def __call__(self, args: Tuple[int, TaskUpdateModel]) -> TaskReadModel:
...
class UpdateTaskUseCaseImpl(UpdateTaskUseCase):
    ...
    async def __call__(self, args: Tuple[int, TaskUpdateModel]) -> TaskReadModel:
        ...
        existing_task = await self.unit_of_work.repository.find_by_id(id_)
        ...
        try:
            updated_task = await self.unit_of_work.repository.update(update_entity)
