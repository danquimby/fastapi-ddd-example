...
class UpdateUserUseCase(BaseUseCase[Tuple[int, UserUpdateModel], UserReadModel]):
    ...
    @abstractmethod
    async def __call__(self, args: Tuple[int, UserUpdateModel]) -> UserReadModel:
...
class UpdateUserUseCaseImpl(UpdateUserUseCase):
    ...
    async def __call__(self, args: Tuple[int, UserUpdateModel]) -> UserReadModel:
        ...
        existing_user: Optional[UserEntity] = (
        ...
        try:
            updated_user = await self.unit_of_work.repository.update(update_entity)
